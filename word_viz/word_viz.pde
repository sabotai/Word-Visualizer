...
//next idea... make a pie chart with slices
//pct of char total determines slice size

//alt: could have different bubble representing different tweets or somethign
//alt2: make it time-based, cycling through various rows

import http.requests.*;
Boolean simulate = true; //fake results or use live request
Boolean shiftMode = false;
Boolean shiftModeTime = false;
...
void mouseClicked(){
  if (mouseButton == RIGHT)
    shiftModeTime = !shiftModeTime;
}

void mousePressed(){
  println("mouseY = " + mouseY);
  println("need to be = " + stdOff * 1.5 + shift);
  if (abs(mouseY - (stdOff * 1.5 + shift)) < 100){
     shiftMode = true;
  println("shiftMode = " + shiftMode);
  } 
...
void draw() {
  ...
  textAlign(CENTER);
  textSize(textSize);
  text(currentHover, width/2, stdOff/2);

  fill(0, 255, 0);
  textSize(60);
  text(bottomText, width/2, height - stdOff/2);
}